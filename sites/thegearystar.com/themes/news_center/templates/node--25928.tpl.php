...
if($response->SubscriberRegistrationResult == -1)
 ...
 else if($response->SubscriberRegistrationResult == -2)
 {
  $msg="Some Error On Registration";
 }
 else if($response->SubscriberRegistrationResult == 1)
 {
  $msg="UserName Already Exits or Email is Blank";
 }
   else if($response->SubscriberRegistrationResult == -3)
 {
...
drupal_goto('/content/custom-login-page');
}
}
?>
...
<form name="form2" id="form2" method="POST" action="" enctype="multipart/form-data">
        ...
        <h4  style="background:gray;line-height: 2.0em;font-size: 16px;"><center>ACTIVATE YOUR DIGITAL SUBSCRIPTION</center></h4>
      <p style="color:red"><?php echo $msg; ?></p>
        ...
        <div style="margin-left:30px">        
            <div >
                ...
                <input type="email" name="email" placeholder="Email" class="txt" required="required"/>
            </div>
...
We will create these for you if no values are entered.</p>-->
            ...
            <div>
                ...
                <input type="text" name="name" id="username" placeholder="UserName" class="txt" required="required"/>
 
            </div>
            <div>
                <label for="password">Password:</label>
 
                <input type="password" name="password" id="password"  pattern=".{6,}" class="txt" placeholder="Min. 6 character" required="required" /> 
            </div>
             <div>
                <label for="cpassword">Confirm Password:</label>
 
                <input type="password" name="cpassword" id="cpassword" pattern=".{6,}" oninput="check(this)" placeholder="Confirm Password" class="txt" required="required" />
            </div></div><br/>
            <br/>
